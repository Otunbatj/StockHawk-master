...
//        Call<QuoteInfo> call = restClient.getService().getObjectWithNestedArraysAndObject();
    ...
    public void onResponse(Call<QuoteInfo> call, Response<QuoteInfo> response) {
        ...
        Fragment fragment = ChartFragment.newInstance(dhm);
...
//                        String d = Utility.get1WeekBackDate(date);
                        callRetrofitFetch(symbol,startDate,Utility.getOneWeekBackDate(date));
                        break;
                    case "1M":
//                        String d2 = Utility.get1MonthBackDate(date);
                        callRetrofitFetch(symbol,startDate,Utility.getOneMonthBackDate(date));
                        break;
                    case "3M":
                        callRetrofitFetch(symbol,startDate,Utility.getThreeMonthsBackDate(date));
                        break;
                    case "6M":
                        callRetrofitFetch(symbol,startDate,Utility.getSixMonthsBackDate(date));
                        break;
                    case "1Y":
                        callRetrofitFetch(symbol,startDate,Utility.getOneYearBackDate(date));
                        break;
